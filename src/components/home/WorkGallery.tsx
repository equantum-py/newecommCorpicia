...
export function WorkGallery({ settings, limit }: { settings: WorkGallerySettings; limit?: number }) {
  // La galería de trabajos no debe mostrarse como preview/resumen en la Home.
  // La galería completa vive únicamente en /proyectos.
  if (limit !== undefined) return null;

  const items = settings.items.filter(item => item.active && item.image);
  ...
  return <section className="bg-[#f6f8f6] py-7 sm:py-12 md:py-14"><div className="container mx-auto px-4">
    ...
    <Link href="/proyectos" className="mt-3 inline-flex items-center gap-1 text-sm font-semibold text-corpicia-green sm:hidden">Ver todos los trabajos <ArrowRight className="h-4 w-4"/></Link>
  </div></section>;
}
