                ...
                return (
                  <article key={project.id} className="border-b pb-10 last:border-b-0 lg:pb-14">
                    <div className="mb-6 grid gap-3 lg:grid-cols-[0.9fr_1.1fr] lg:items-end">
                      ...
                      {project.description && <p className="max-w-2xl text-sm leading-6 text-gray-600 sm:text-base lg:justify-self-end">{project.description}</p>}
                    </div>
                    ...
                    <div className="space-y-6 lg:space-y-7">
                      {stages.map(stage => {
                        ...
                        return <section key={stage}>
                          <div className="mb-3 flex items-center gap-3"><span className="flex h-7 w-7 items-center justify-center rounded-full bg-corpicia-green text-[11px] font-bold text-white">{info.number}</span><h3 className="text-sm font-bold uppercase tracking-wide text-gray-900 sm:text-base">{info.label}</h3><div className="h-px flex-1 bg-gray-200" /></div>
                          <div className={`grid gap-3 sm:gap-4 ${stagePhotos.length === 1 ? 'grid-cols-1 lg:grid-cols-2' : 'grid-cols-2 md:grid-cols-3'}`}>
                            {stagePhotos.map((photo, index) => <figure key={photo.id} className={`relative overflow-hidden rounded-2xl bg-gray-100 ${stagePhotos.length === 1 ? 'aspect-[4/3] w-full lg:max-w-[620px]' : 'aspect-[4/3]'}`}><Image src={photo.url} alt={`${project.title} - ${info.label} ${index + 1}`} fill className="object-cover" sizes={stagePhotos.length === 1 ? '(max-width:1023px) 100vw, 620px' : '(max-width:767px) 50vw,33vw'} /><figcaption className="absolute bottom-3 left-3 rounded-lg bg-black/70 px-3 py-1.5 text-xs font-bold uppercase tracking-wider text-white">{info.label}{stagePhotos.length > 1 ? ` ${index + 1}` : ''}</figcaption></figure>)}
                          </div>
                        </section>;
                      })}
                    </div>
