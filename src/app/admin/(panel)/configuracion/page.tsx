...
export default function AdminConfiguracionPage() {
  ...
  return <div className="mx-auto max-w-5xl space-y-5 pb-12">
    ...
    <Card><CardContent className="space-y-6 pt-6">
      ...
      {data.mode==='banner' && <div className="grid gap-5 border-t pt-5 sm:grid-cols-2">{(['desktop','mobile'] as const).map(target=><div key={target}><p className="font-bold">Banner {target==='desktop'?'Desktop':'Mobile'}</p><p className="mb-2 text-xs text-gray-500">{target==='desktop'?'1920 × 650 px':'1080 × 1350 px'} · JPG, PNG o WebP · máx. 5 MB</p><label className="flex min-h-40 cursor-pointer items-center justify-center overflow-hidden rounded-xl border-2 border-dashed bg-gray-50">{data[target==='desktop'?'desktopImage':'mobileImage']?<img src={data[target==='desktop'?'desktopImage':'mobileImage']} alt="Vista previa" className="h-48 w-full object-cover"/>:<span className="text-center text-sm font-semibold text-gray-600"><UploadCloud className="mx-auto mb-2 h-7 w-7"/>{uploading===target?'Subiendo...':'Seleccionar imagen'}</span>}<input type="file" accept="image/png,image/jpeg,image/webp" className="hidden" disabled={!!uploading} onChange={e=>upload(e.target.files?.[0],target)}/></label></div>)}</div>}
